...
FightDisplay::FightDisplay(QWidget *parent) :
    QWidget(parent),
    displayUi(new Ui::FightDisplay),
    splashUi(new Ui::Splash)
{
    initPalette();
    ...
    displayUi->countryFlagLayout->addWidget(redFlagWidget, 50, Qt::AlignRight | Qt::AlignVCenter);
    displayUi->countryFlagLayout->addWidget(displayUi->redCountryLabel, 50, Qt::AlignRight | Qt::AlignVCenter);

    blueFlagWidget = new QSvgWidget(display);
    blueFlagWidget->hide();

    displayUi->countryFlagLayout->addWidget(displayUi->blueCountryLabel, 50, Qt::AlignLeft | Qt::AlignVCenter);
    displayUi->countryFlagLayout->addWidget(blueFlagWidget, 50, Qt::AlignLeft | Qt::AlignVCenter);
...
void FightDisplay::setFontsSize()
{
    ...
    displayUi->penLabel->setFont(font6);
...
void FightDisplay::setRedFlag(const QString &filename)
{
    if ( filename.isEmpty() ) {
        redFlagWidget->hide();
...
void FightDisplay::setBlueFlag(const QString &filename)
{
    if ( filename.isEmpty() ) {
        blueFlagWidget->hide();
