...
namespace Melampig
{
    WrestlerCompSheet::WrestlerCompSheet(Object *o, Keeper *keeper, QWidget *parent) :
    ...
    void WrestlerCompSheet::genCompetition()
    {
        ...
        pVars.insert("{competition}", object->get("title"));

        pVars.insert("{date}",   date);
        pVars.insert("{weight}", w->get("title"));
        pVars.insert("{geo}",    g->get("title"));
        ...
        opts.insert("weight", keeper->prepareParam(And|Equal, wid));
        opts.insert("style", keeper->prepareParam(And|Equal, sid));
        opts.insert("cround", keeper->prepareParam(And|Not|InSet, rrounds));
        ...
        {
            int level = 0;
            bool last = false; // last coll on page
            ...
            for( int i = 0; i < fnum; i++ )
            {
                ...
                if ( !fightsMap.contains(t) )
                {
                    ...
                    rows << applyTemplateVars( j == 0 ? fightQualify_tmpl :
                                                 last ? fightRoundLast_tmpl :
                                                        fightRound_tmpl,
                                               tVars);
                    tVars.clear();
                }
                else
                {
                    ...
                    if ( j == (tourList.size()-1) )
                    {
                        ...
                        if ( wrestler ) {
                            ...
                            tVars.insert("{winner.num}", "");
                            tVars.insert("{winner.fio}", "");
                            tVars.insert("{winner.geo}", "");
                        }
    ...
    void WrestlerCompSheet::genRepechage()
    {
        int sid = styleCombo->itemData (styleCombo->currentIndex()).toInt();
        int wid = weightCombo->itemData(weightCombo->currentIndex()).toInt();
        int cid = competitionCombo->itemData(competitionCombo->currentIndex()).toInt();
        ...
        if ( !query.exec() )
            QMessageBox::critical(this, tr("Error"), tr("Could not get wrestlers count\n") + query.executedQuery() );
        ...
        for( int j = 0; j < roundList.size(); j++ )
        {
            ...
            for( int i = 0; i < repSize ; i++ )
            {
                ...
                pVars.insert( QString("{repechage:%1:%2}").arg( j == 0 ? "up" : "low").arg(i+1),
                              applyTemplateVars(tmpl, tVars) );
                tVars.clear();
            }
        }
    ...
    void WrestlerCompSheet::genClassement()
    {
        // CLASSIFY
        ...
        bool show_points = pointsBox->isChecked();
    ...
    QString WrestlerCompSheet::generateData()
    {
        setReportFile("index.html");
        setReportPath( QString("/../share/reports/%1/wrestler/competition").arg(reportLang) );
